...
export const syncGoogleSheetData = async () => {
  try {
    ...
    const nameIndex = headers.findIndex((header: string) => header.toLowerCase().includes("name"))
    const emailIndex = headers.findIndex((header: string) => header.toLowerCase().includes("email"))
    const phoneIndex = headers.findIndex((header: string) => header.toLowerCase().includes("phone"))
    const sourceIndex = headers.findIndex((header: string) => header.toLowerCase().includes("source"))
...
export const startGoogleSheetSync = () => {
  ...
  console.log(`Google Sheet sync scheduled to run every ${syncInterval / 60000} minutes`)
}
